...
class BL410ModbusReader {
     ...
     */
    async connect(port = '/dev/ttyS0', baudRate = 9600, parity = 'none') {
        try {
     ...
     */
    registers32ToInt(registers, startIndex) {
     ...
     */
    async readCompleteFlowmeterData(deviceAddress) {
        if (!this.isConnected) {
            throw new Error('Not connected to RS485. Call connect() first.');
        }
        try {
     ...
     */
    async resetFlowmeterAccumulation(deviceAddress) {
        if (!this.isConnected) {
            throw new Error('Not connected to RS485. Call connect() first.');
        }
        try {
     ...
     */
    async readHoldingRegisters(deviceId, address, count) {
        if (!this.isConnected) {
            throw new Error('Not connected to RS485. Call connect() first.');
        }
        try {
     ...
     */
    async readInputRegisters(deviceId, address, count) {
        if (!this.isConnected) {
            throw new Error('Not connected to RS485. Call connect() first.');
        }
        try {
     ...
     */
    async readCoils(deviceId, address, count) {
        if (!this.isConnected) {
            throw new Error('Not connected to RS485. Call connect() first.');
        }
        try {
...
async function main() {
    ...
    try {
        ...
        console.log('');
        // Connect to RS485
        await reader.connect('/dev/ttyS0', 115200, 'none');
