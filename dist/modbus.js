...
exports.BL410ModbusReader = void 0;
...
class BL410ModbusReader {
     ...
     */
    async connect(port = '/dev/ttyS0', baudRate = 9600, parity = 'even') {
        try {
    ...
    static async listSerialPorts() {
        try {
            ...
            ports.forEach((port, index) => {
                ...
                if (port.pnpId)
                    console.log(`      🏷️  PnP ID: ${port.pnpId}`);
                console.log('');
            });
        }
        catch (error) {
     ...
     */
    createReadHoldingRegistersFrame(address, startRegisterAddr, quantity) {
        // Validate inputs
    ...
    verifyModbusCRC(frameData) {
        ...
        if (calculatedCrc === receivedCrc) {
            if (this.debugMode)
                console.log('✅ CRC validation passed (standard byte order)');
            return true;
        }
        ...
        if (calculatedCrc === receivedCrcReversed) {
            if (this.debugMode)
                console.log('✅ CRC validation passed (reversed byte order)');
            return true;
        }
     ...
     */
    async sendRawModbusFrame(frame, timeoutMs = 3000) {
        if (!this.isConnected || !this.serialPort) {
     ...
     */
    async testDeviceResponsive(deviceAddress) {
        if (!this.isConnected) {
            return false;
        }
        const maxAttempts = 3;
     ...
     */
    async readFlowmeterData(deviceAddress) {
        if (!this.isConnected) {
            throw new Error('Not connected to RS485. Call connect() first.');
        }
        try {
     ...
     */
    async scanForDevices(addressRange = [1, 2, 3, 4, 5]) {
     ...
     */
    async monitorFlowmeters(deviceAddresses, intervalMs = 5000) {
...
exports.BL410ModbusReader = BL410ModbusReader;
...
async function main() {
    ...
    try {
        ...
        readline.question('\nStart continuous flowmeter monitoring? (y/n): ', (answer) => {
            readline.close();
